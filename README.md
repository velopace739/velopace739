<img src="https://ipfs.io/ipfs/bafybeieje5dslabol4yi3oetvfg5gmmoluunrsims2k45e7lfvcxhciqry">

<h1 align="center"><img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="28">Full Stack Web3 Developer</h1>

<h4 align="center">Visitor's count :eyes:</h4>
<p align="center"><img src="https://api.visitorbadge.io/api/visitors?path=velopace739&label=Visitors&countColor=#263759" alt="velopace739 :: Visitor's Count" /></p>

<p align='center'>
<img src='https://github-widgetbox.vercel.app/api/profile?username=velopace739&data=followers,repositories,stars,commits' />
</p>

# Blockchain skills
```
  - Smart contracts, Solidity
  - Web3.js, Ethers.js
  - dApps
  - NFT, ERC721, ERC1155
  - Truffle, Hardhat, Foundry
  - Ganache, waffle
  - Infura, Alchemy, Moralis, Thirdweb
```
# Frontend skills
```
  - React.js, React Native, Next.js, Vue.js, WordPress, Redux-Saga/Redux-Thunk
  - Svelte, Angular, Lit
  - HTML5, CSS3, Bootstrap, Styled components, LESS / SASS
  - JavaScript, jQuery, Ajax
  - TypeScript
  - Ant Design, Material Design, ShadCN 
  - WebPack, NPM, Yarn
```
# Backend skills
``` 
  - Node.js / Express / RESTful APIs, GraphQL
  - Java, Spring Boot, Dropwizard, Micronut
  - Python, Fast API, Django, Flask
  - JWT, OAuth2, Data Encryption, Secure API Design, XSS/CSRF Protection
  - MySQL, MongoDB, PostgreSQL, Redis
  - Docker, Kubernetes, Jenkins, GitLab CI
  - AWS (EC2, S3, Lambda, RDS), GCP (Cloud functions, API Gateway, Tasks, Firebase)
  - Microservices, Serverless Architecture
```
